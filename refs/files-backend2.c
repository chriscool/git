...
/*
 * Dummy backend copied from files-backend.c
 *
 * The goal of this backend is to see which tests are breaking when
 * using a file backend that uses "refs2/" instead of "refs/" to store
 * refs.
 */
			       ...
			       struct ref_dir *dir, const char *dirname)
{
	...
	/*
	 * Manually add refs2/bisect, which, being per-worktree, might
	 * not appear in the directory listing for refs2/ in the main
	 * repo.
	 */
	if (!strcmp(dirname, "refs2/")) {
		int pos = search_ref_dir(dir, "refs2/bisect/", 12);

		if (pos < 0) {
			struct ref_entry *child_entry = create_dir_entry(
					dir->cache, "refs2/bisect/", 12, 1);
...
static struct ref_cache *get_loose_ref_cache(struct files_ref_store *refs)
{
	if (!refs->loose) {
		...
		/*
		 * Add an incomplete entry for "refs2/" (to be filled
		 * lazily):
		 */
		add_entry_to_dir(get_ref_dir(refs->loose->root),
				 create_dir_entry(refs->loose, "refs2/", 6, 1));
	}
	return refs->loose;
}
...
stat_ref:
	...
	/* Follow "normalized" - ie "refs2/.." symlinks by hand */
	if (S_ISLNK(st.st_mode)) {
		...
		if (starts_with(sb_contents.buf, "refs2/") &&
...
retry:
	...
	case SCLD_EXISTS:
		/*
		 * Suppose refname is "refs2/foo/bar". We just failed
		 * to create the containing directory, "refs2/foo",
		 * because there was a non-directory in the way. This
		 * indicates a D/F conflict, probably because of
		 * another reference such as "refs2/foo". There is no
	...
	if (files2_read_raw_ref(&refs->base, refname,
		...
		if (errno == ENOENT) {
			if (mustexist) {
				 ...
				 * reference "refs2/foo/bar"):
				 *
				 * - We were successfully able to create
				 *   the lockfile refs2/foo/bar.lock, so we
				 *   know there cannot be a loose reference
				 *   named "refs2/foo".
				 *
				 * - We got ENOENT and not EISDIR, so we
				 *   know that there cannot be a loose
				 *   reference named "refs2/foo/bar/baz".
				 */
			}
				     ...
				     unsigned int flags)
{
	...
	p = buf.buf;
	for (i = 0; i < 2; i++) { /* refs2/{heads,tags,...}/ */
		while (*p && *p != '/')
			p++;
			   ...
			   unsigned int pack_flags)
{
	...
	if (!(pack_flags & PACK_REFS_ALL) && !starts_with(refname, "refs2/tags/"))
		return 0;
 ...
 * live into logs/refs.
 */
#define TMP_RENAMED_LOG  "refs2/.tmp-renamed-log"
