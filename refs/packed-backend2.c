 ...
 * `packed_ref2_store`. Its freshness is checked whenever
 * `get_snapshot()` is called; if the existing snapshot is obsolete, a
 * new snapshot is taken.
 */
struct snapshot {
	/*
	 * A back-pointer to the packed_ref2_store with which this
	 ...
	 * from `packed_ref2_store::snapshot`, if any. The instance
...
/*
 * Downcast `ref_store` to `packed_ref2_store`. Die if `ref_store` is
 * not a `packed_ref2_store`. Also die if `packed_ref2_store` doesn't
 * support at least the flags specified in `required_flags`. `caller`
 * is used in any necessary error messages.
 */
static struct packed_ref2_store *packed_downcast(struct ref_store *ref_store,
						...
						const char *caller)
{
	...
	refs = (struct packed_ref2_store *)ref_store;
...
static void clear_snapshot(struct packed_ref2_store *refs)
{
	if (refs->snapshot) {
...
static int cmp_packed_ref2_records(const void *v1, const void *v2)
{
...
static void sort_snapshot(struct snapshot *snapshot)
{
	...
	while (pos < eof) {
		...
		if (sorted &&
		    nr > 1 &&
		    cmp_packed_ref2_records(&records[nr - 2],
					   &records[nr - 1]) >= 0)
			sorted = 0;
	...
	QSORT(records, nr, cmp_packed_ref2_records);
 ...
 */
static struct snapshot *create_snapshot(struct packed_ref2_store *refs)
{
...
/*
 * Get the `snapshot` for the specified packed_ref2_store, creating and
 ...
 */
static struct snapshot *get_snapshot(struct packed_ref2_store *refs)
{
 ...
 * of `ITER_DONE`.
 */
static int next_record(struct packed_ref2_iterator *iter)
{
...
static int packed_ref2_iterator_advance(struct ref_iterator *ref_iterator)
{
	struct packed_ref2_iterator *iter =
		(struct packed_ref2_iterator *)ref_iterator;
	int ok;
...
static int packed_ref2_iterator_peel(struct ref_iterator *ref_iterator,
...
static int packed_ref2_iterator_abort(struct ref_iterator *ref_iterator)
{
	struct packed_ref2_iterator *iter =
		(struct packed_ref2_iterator *)ref_iterator;
	int ok = ITER_DONE;
...
static struct ref_iterator_vtable packed_ref2_iterator_vtable = {
	packed_ref2_iterator_advance,
	packed_ref2_iterator_peel,
	packed_ref2_iterator_abort
};

static struct ref_iterator *packed_ref2_iterator_begin(
		...
		const char *prefix, unsigned int flags)
{
	...
	base_ref_iterator_init(ref_iterator, &packed_ref2_iterator_vtable, 1);
...
void packed_refs2_unlock(struct ref_store *ref_store)
{
	...
	if (!is_lock_file_locked(&refs->lock))
		BUG("packed_refs2_unlock() called when not locked");
	rollback_lock_file(&refs->lock);
}
 ...
 */
static const char PACKED_REFS2_HEADER[] =
 ...
 */
static int write_with_updates(struct packed_ref2_store *refs,
			      ...
			      struct strbuf *err)
{
	 ...
	 */
	packed_refs2_path = get_locked_file_path(&refs->lock);
	strbuf_addf(&sb, "%s.new", packed_refs2_path);
	free(packed_refs2_path);
	...
	if (fprintf(out, "%s", PACKED_REFS2_HEADER) < 0)
		goto write_error;
	 ...
	 */
	iter = packed_ref2_iterator_begin(&refs->base, "",
...
static void packed_transaction_cleanup(struct packed_ref2_store *refs,
				       struct ref_transaction *transaction)
{
				      ...
				      struct strbuf *err)
{
	...
	if (!is_lock_file_locked(&refs->lock)) {
		if (packed_refs2_lock(ref_store, 0, err))
			goto failure;
		data->own_lock = 1;
	}
				     ...
				     struct strbuf *err)
{
	...
	packed_refs2_path = get_locked_file_path(&refs->lock);
	if (rename_tempfile(&refs->tempfile, packed_refs2_path)) {
