...
/*
 * List of field names that can be used in the "promisor-remote"
 * protocol capability (others must be ignored). Each field should
 * correspond to a configurable property of a remote that can be
...
{
	if (!strcasecmp(field, "partialCloneFilter"))
		p->filter = xstrdup(value);
	else if (!strcasecmp(field, "token"))
 ...
 * remotes. Only members of that struct specified by the 'field_names'
 * linked list are set (using values from the configuration).
 */
...
char *promisor_remote_info(struct repository *repo)
{
	...
	for (p = info_list; p; p = p->next) {
		...
		if (p->filter) {
			strbuf_addstr(&sb, ",partialCloneFilter=");
			strbuf_addstr_urlencode(&sb, p->filter, allow_unsanitized);
		}
		if (p->token) {
			strbuf_addstr(&sb, ",token=");
...
static int is_matching_field(struct string_list_item *item_value,
			     struct promisor_info *p)
{
	const char *field_name = item_value->string;
	const char *field_value = item_value->util;

	if (p->filter && !strcasecmp(field_name, "partialCloneFilter"))
		return !strcmp(p->filter, field_value);
	else if (p->token && !strcasecmp(field_name, "token"))
		return !strcmp(p->token, field_value);

	return 0;
}

static int has_matching_field(struct string_list_item *item_value,
			      struct promisor_info *p, int in_list)
{
	if (!in_list)
		return is_matching_field(item_value, p);

	for (; p; p = p->next)
		if (is_matching_field(item_value, p))
			return 1;

	return 0;
}

static int all_fields_match(struct string_list* values,
			    struct promisor_info *p,
			    int in_list)
{
	...
	for_each_string_list_item(item_checked, fields) {
		...
		if (!item_value)
			return 0;
		if (!has_matching_field(item_value, p, in_list))
			return 0;
	}
				...
				struct promisor_info *info_list)
{
	...
	if (accept == ACCEPT_ALL)
		return all_fields_match(values, info_list, 1);
	...
	if (accept == ACCEPT_KNOWN_NAME)
		return all_fields_match(values, p, 0);
	...
	if (!p->url)
		BUG("Bad info_list (invalid URL) for remote '%s'.\n",
		    remote_name);
