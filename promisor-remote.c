...
static int promisor_remote_config(const char *var, const char *value, void *data)
{
	...
	if (!strcmp(subkey, "promisor")) {
		...
		if (do_find_promisor_remote(remote_name, NULL)) {
			free(remote_name);
	...
	if (!strcmp(subkey, "partialclonefilter")) {
		...
		o = do_find_promisor_remote(remote_name, NULL);
		if (!o)
...
static void promisor_remote_do_init(int force)
{
	...
	if (repository_format_partial_clone) {
		...
		o = do_find_promisor_remote(repository_format_partial_clone,
					    &previous);
		if (o)
...
struct promisor_remote *find_promisor_remote(const char *remote_name)
{
	...
	return do_find_promisor_remote(remote_name, NULL);
}
