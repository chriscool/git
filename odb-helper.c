...
static int get_direct_object_script(struct odb_helper *o, const unsigned char *sha1)
{
	struct odb_helper_cmd cmd;
			  ...
			  const unsigned char *sha1)
{
	...
	if (o->script_mode)
		res = get_direct_object_script(o, sha1);
	else
...
static int get_object_script(struct odb_helper *o, const unsigned char *sha1, int fd)
{
	...
	// TODO maybe use
	//	BUG("invalid get capability '%d'", o->supported_capabilities);
	return -1;
}
