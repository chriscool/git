...
test_expect_success "setup for subsequent fetches" '
	...
	git -C server fetch origin master &&
	# git -C server reset --keep FETCH_HEAD &&
	git -C server update-ref HEAD FETCH_HEAD &&
	git -C server rev-parse master >expected_master &&
	...
	initialize_server 2 "$(cat expected_missing.txt)" &&
...
test_expect_success "subsequent fetch from a client when promisor.advertise is true" '
	...
	git -C client rev-parse master >actual &&
	test_cmp expected_master actual &&

	cat client/bar >/dev/null &&

	check_missing_objects server 2 "$(cat expected_missing.txt)"
'
...
test_expect_success "subsequent fetch from a client when promisor.advertise is false" '
	...
	git -C client2 rev-parse master >actual &&
	test_cmp expected_master actual &&
