...
test_description='basic tests for transfering remote ODBs'
...
put_raw_obj)
	...
	ref_hash=$(echo "$sha1 $size $kind" | GIT_DIR=$ORIG_SOURCE GIT_NO_REMOTE_ODB=1 git hash-object -w -t blob --stdin) || exit
...
put_raw_obj)
	...
	writen=$(git hash-object -w -t "$kind" --stdin)
	test "$writen" = "$sha1" || die "bad sha1 passed '$sha1' vs writen '$writen'"
	ref_hash=$(echo "$sha1 $size $kind" | GIT_DIR=$OTHER_SOURCE GIT_NO_REMOTE_ODB=1 git hash-object -w -t blob --stdin) || exit
