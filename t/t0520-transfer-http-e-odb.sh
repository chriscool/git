...
test_description='tests for transfering remote objects to an HTTPD server'
...
put_raw_obj)
	...
	ref_hash=$(echo "$sha1 $size $kind" | GIT_NO_REMOTE_ODB=1 git hash-object -w -t blob --stdin) || exit
