...
uploadpack.lazyFetchTrusted::
	...
	`upload-pack` that it is trusted, so lazy fetching from promisor
	remotes that have been advertised by the server and accepted by the
	client using the "promisor-remote" protocol v2 capability (see
	linkgit:gitprotocol-v2[5]) is allowed for it. This is equivalent to
	setting `GIT_NO_LAZY_FETCH` to `fromAccepted` for the matching
	repositories, and only takes effect for protocol v2. An explicitly
	set `GIT_NO_LAZY_FETCH` takes precedence over this setting.
+
